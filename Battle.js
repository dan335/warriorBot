...
export default class Battle {
  constructor(db, discord, msg, warrior1, warrior2, user1, user2) {
    ...
    if (this.warrior1.died) {
      ...
      functions.killWarrior(db, discord, this.warrior1, 'was killed by '+this.warrior2.name+'.');
    }
  ...
  printToDiscord() {
    ...
    guildsCollection.findOne({discordId:this.user1.guildDiscordId}, {}, (error, guild) => {
      if (error) {
        ...
        if (guild) {
          functions.sendToChannel(this.discord, guild.channelId, ':crossed_swords: '+this.shortDescription);
        }
      }
    });
  }
